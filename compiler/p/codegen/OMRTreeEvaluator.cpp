...
TR::Register * OMR::Power::TreeEvaluator::ibyteswapEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
   ...
   if (reverseLoadEnabled && !firstChild->getRegister() &&
      ...
      if (cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P10))
         ...
         {
         generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwimi, node, tgtRegister, srcRegister, 24, 0x00ffffff00);
   ...
   if (comp->target().is64Bit())
      ...
      if (reverseLoadEnabled && comp->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P7) &&
         ...
         if (comp->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P10))
            ...
            generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwimi, node, tmpRegister, srcLRegister, 24, 0x00ffffff00);
            generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwimi, node, tgtRegister, srcHRegister, 24, 0x00ffffff00);
      ...
      if (comp->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P10))
         ...
         {
         generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwimi, node, tgtRegister->getLowOrder(), srcRegister->getHighOrder(), 24, 0x00ffffff00);
         generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwimi, node, tgtRegister->getHighOrder(), srcRegister->getLowOrder(), 24, 0x00ffffff00);
