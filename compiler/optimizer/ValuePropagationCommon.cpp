   ...
   if (TR::Compiler->om.isOffHeapAllocationEnabled())
      ...
      if (offHigh > 0)
         ...
         if (elementSize == 1)
            ...
            array = TR::TransformUtil::generateArrayOffsetTrees(comp, offNode, stride, elementSize, true);
            }
         else
            {
            array = TR::TransformUtil::generateArrayOffsetTrees(comp, offNode, stride, elementSize, false);
            }
         }
      array = TR::TransformUtil::generateArrayAddressTrees(comp, objNode, array);
...
*/
   ...
   if (TR::Compiler->om.isOffHeapAllocationEnabled())
      ...
      if (encode)
         {
         srcOff = TR::TransformUtil::generateArrayOffsetTrees(comp(), srcOff, strideNode, 0, false);
         arrayTranslateNode->setSourceIsByteArrayTranslate(false);
         arrayTranslateNode->setTargetIsByteArrayTranslate(true);
         }
      else
         {
         dstOff = TR::TransformUtil::generateArrayOffsetTrees(comp(), dstOff, strideNode, 0, false);
         arrayTranslateNode->setSourceIsByteArrayTranslate(true);
         arrayTranslateNode->setTargetIsByteArrayTranslate(false);
         }
      src = TR::TransformUtil::generateArrayAddressTrees(comp(), srcObj, srcOff);
      dst = TR::TransformUtil::generateArrayAddressTrees(comp(), dstObj, dstOff);
      }
   else
#endif /* OMR_GC_SPARSE_HEAP_ALLOCATION */
      {
...
void OMR::ValuePropagation::transformArrayCloneCall(TR::TreeTop *callTree, OMR::ValuePropagation::ArrayCloneInfo *cloneInfo)
   ...
   if (TR::Compiler->om.isOffHeapAllocationEnabled())
      {
      lengthInBytes = TR::TransformUtil::generateArrayOffsetTrees(comp(), lenNode, NULL, elementSize, false);
      srcStart = TR::TransformUtil::generateArrayStartTrees(comp(), objNode);
      destStart = TR::TransformUtil::generateArrayStartTrees(comp(), newArray);
      }
   else
#endif /* OMR_GC_SPARSE_HEAP_ALLOCATION */
      {
      lengthInBytes = comp()->target().is64Bit() ?
